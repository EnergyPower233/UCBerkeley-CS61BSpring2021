 ...
 *
 * @author E_Power, for UCB CS61B Project0
 */
public class Model extends Observable {
     ...
     *
     */
    private boolean checkContains(int[] list, int x) {
    ...
    private boolean tilt1col(int col) {
        boolean changed = false;
        int[] mergedblock = new int[board.size()/2];
        for(int _l = 0; _l < mergedblock.length; ++_l) {
            mergedblock[_l] = -1;
        }
        int index = 0;
        ...
        for (int i = board.size() - 2; i >= 0; --i) {
            ...
            for (int j = i + 1; j < board.size(); ++j) {
                if (board.tile(col, j) == null) {
                    continue;
                } else if (board.tile(col, j).value() == t.value()) {
                    //check if the blocks has merged
                    if (checkContains(mergedblock, j)) {
